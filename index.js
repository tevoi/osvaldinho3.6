		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Tevo BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Tevo? Este é um recurso especial para o Tevo ❌',
					ownerB: '[❗] Tevo? Este é um recurso especial para o Tevo ❌',
			...
			const ownerNumber = ["555491057713@s.whatsapp.net"] //número do dono
			const mod = [ownerNumber,"555491057713@s.whatsapp.net"]//mod
			const adminbotnumber = ["555491057713@s.whatsapp.net"]//adm
			const frendsowner = ["554792091566@s.whatsapp.net"]//amigos 
			const premium = ["555491057713@s.whatsapp.net","554199856977@s.whatsapp.net"]//Premium 
...
case 'sc':
case 'run':
if (!isOwner) return reply('so o dono 😜')
					    try {
...
break                   
				...
				case 'ytmp3':
                 ...
                 case 'nomegp':
		            reply (`o nome do grupo é: *${groupName}*`)
		             break
				...
				case 'pokemon':
					...
					case 'vipmenu':
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *#daftarvip* para comprar acesso Premium!' ,text, { quoted: msg })
				...
				case 'premiumlist':
					...
					break
                  case 'qrcode':
        			if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: msg })
					const tex = encodeURIComponent(body.slice(8))
					if (!tex) return client.sendMessage(from, 'Digite um texto/url que deseja criar um código qr', text, {quoted: msg})
		        ...
		        case 'wame':
                 ...
                 case 'setdesc':
				     ...
				     client.sendMessage(from, 'Descrição alterada com sucesso', text, {quoted: msg})
				     break
			case 'google':
			...
			case 'closegp':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'sticker':
					...
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem, tio?', text, {quoted: msg})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto tio', text, {quoted: msg})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Causs ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: msg })
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Tevo ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: msg })
					if (msg.message.extendedTextMessage === undefined || msg.message.extendedTextMessage === null) return reply('A marca-alvo que você quer chutar!')
					mentioned = msg.message.extendedTextMessage.contextInfo.mentionedJid
					mentioned = msg.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: msg, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/555491057713/*╰──────*「 * TEVO* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO Osvaldinho_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: msg, caption: 'Bom dia, vcs sao fodas ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: msg, caption: 'Boa tarde, rapeize 😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: msg, caption: 'Boa noite fml ❤️'})
					break
                
				case 'lofi':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTL9hZBPRo16fIhsIus3t1je2oAU23pQqBpfw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: msg, caption: '️💆'})
					break
			
				case 'canal':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/xuTCBPO`)
					client.sendMessage(from, buffer, image, {quoted: msg, caption: '️*canal do Causs:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/4HeRfuO`)
					client.sendMessage(from, buffer, image, {quoted: msg, caption: 'bom dia ❤️'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: msg, caption: '*CRIADOR:* Tevo\n*WPP:* wa.me/+555491057713\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
